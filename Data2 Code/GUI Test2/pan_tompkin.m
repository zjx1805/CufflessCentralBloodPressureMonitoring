...
% impossible for a legitimate QRS complex to occur if it lies within 300ms after a previously
...
if gr
 if fs == 200
  figure(2); clf; ax(1)=subplot(321);plot(ecg);axis tight;title('Raw ECG Signal');
 else
  figure(2); clf; ax(1)=subplot(3,2,[1 2]);plot(ecg);axis tight;title('Raw ECG Signal');
 end
end    
...
% 300 msec distance
[pks,locs] = findpeaks(ecg_m,'MINPEAKDISTANCE',round(0.350*fs));
  ...
  %% update the heart_rate (Two heart rate means one the moste recent and the other selected)
    ...
    if test_m
          if (locs(i) - qrs_i(end)) >= round(1.66*test_m)% it shows a QRS is missed 
              [pks_temp,locs_temp] = max(ecg_m(qrs_i(end)+ round(0.350*fs):locs(i)-round(0.350*fs))); % search back and locate the max in this interval
              locs_temp = qrs_i(end)+ round(0.350*fs) + locs_temp -1; %location 
              ...
              if pks_temp > THR_NOISE
               ...
               if y_i_t > THR_NOISE1 
                      ...
                      SIG_LEV1 = 0.5*y_i_t + 0.5*SIG_LEV1; %when found with the second thres 
               end
               
               not_nois = 1;
               SIG_LEV = 0.5*pks_temp + 0.5*SIG_LEV ;  %when found with the second threshold             
             end 
    ...
    if pks(i) >= THR_SIG
                 ...
                 if length(qrs_c) >= 3
                      if (locs(i)-qrs_i(end)) <= round(0.460*fs)
                        ...
                        Slope2 = mean(diff(ecg_m(qrs_i(end)-round(0.075*fs):qrs_i(end)))); %mean slope of previous R wave
                             if abs(Slope1) <= abs(0.9*(Slope2))  % slope less then 0.5 of previous R
                                 nois_c = [nois_c pks(i)];
...
if gr
figure(2);
az(1)=subplot(311);plot(ecg_h);title('QRS on Filtered Signal');axis tight;

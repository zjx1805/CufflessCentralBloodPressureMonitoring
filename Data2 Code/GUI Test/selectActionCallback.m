...
%     findCandidateFeature(signal,signalName,isForBCG,eventTag,mPlots,mParams)
    ...
    analyze(entryCode,eventTag,mAxes,mPlots,mControls,mParams)
elseif strcmp(actionName,'Go To I-J Window') == 1
    allShades = findobj(mPlots.Analysis.(eventTag).Parent,'Tag','Shade');
    delete(allShades)
    IJWindow = getappdata(mPlots.Analysis.(eventTag).Parent,['IJWindow']);
    if any(isnan(IJWindow))
        fprintf(['BCG does not have enough detected features!\n'])
    else
        featureStartIdx = IJWindow(1); featureEndIdx = IJWindow(2);
        featureMaxLoc = getappdata(mPlots.Analysis.(eventTag).Parent,['BCGMaxLoc']);
        ...
        BCGsigYlim = mAxes.Analysis.(eventTag).BCG.YLim;
        mPlots.Analysis.(eventTag).BCGShade = rectangle(mAxes.Analysis.(eventTag).BCG,...
...
elseif strcmp(actionName,'Go To Feature Window') == 1
    if contains(signalName,'PPG') == 1
        ...
        PPGsigWindow = getappdata(mPlots.Analysis.(eventTag).Parent,[signalName,referenceSignalName,'Window']);

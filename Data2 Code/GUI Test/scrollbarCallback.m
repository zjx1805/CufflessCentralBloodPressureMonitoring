function []=scrollbarCallback(source,callbackdata,mAxes,mPlots,mControls,mParams)

% Redraws the data points on screen when scroll slider is moved.

fs = mParams.Constant.fs;
scrollLoc = source.Value;
eventTag = source.Tag;
...
panLoc = mControls.Analysis.(eventTag).hPanSlider.Value;
...
%     if strcmp(featureName,'BCGMax') == 1
%         featureXData
%     end

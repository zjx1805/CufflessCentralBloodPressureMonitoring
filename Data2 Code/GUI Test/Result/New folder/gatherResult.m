...
allIndexes = [8,9,10,12,13,14,19,20,21,22,24,27,28];
SPTotal = []; DPTotal = [];
for ii = 1:length(allIndexes)
    currentIdx = allIndexes(ii);
    load(['Subject' num2str(currentIdx) 'Data.mat'])
    SPTotal(1:13,currentIndex)=[PAT.PPGfingerECG.correlation(1);PAT.PPGearECG.correlation(1);PAT.PPGfeetECG.correlation(1);PAT.PPGtoeECG.correlation(1);
    ...
    DPTotal(1:13,currentIndex)=[PAT.PPGfingerECG.correlation(2);PAT.PPGearECG.correlation(2);PAT.PPGfeetECG.correlation(2);PAT.PPGtoeECG.correlation(2);
